component{
	...
	private void function overrideDefaultDateFormats( required struct formats ){
		for( var format in arguments.formats ){
			if( !variables.dateFormats.KeyExists( format ) )
				Throw( type=variables.exceptionType, message="Invalid date format key", detail="'#format#' is not a valid dateformat key. Valid keys are DATE, DATETIME, TIME and TIMESTAMP" );
	...
	public struct function getEnvironment(){
		return {
			dateFormats: variables.dateFormats
			,engine: server.coldfusion.productname & " " & ( variables.isACF? server.coldfusion.productversion: ( server.lucee.version?: "?" ) )
			,engineSupportsEncryption: variables.engineSupportsWriteEncryption //for backwards compat only //TODO remove on next major version
			,engineSupportsWriteEncryption: variables.engineSupportsWriteEncryption
			,javaLoaderDotPath: variables.javaLoaderDotPath
			,javaClassesLastLoadedVia: variables.javaClassesLastLoadedVia
			,javaLoaderName: variables.javaLoaderName
			,requiresJavaLoader: variables.requiresJavaLoader
	...
	public function csvToQuery(
		...
		if( !csvIsString AND !csvIsFile )
			Throw( type=variables.exceptionType, message="Missing required argument", detail="Please provide either a csv string (csv), or the path of a file containing one (filepath)." );
		if( csvIsString AND csvIsFile )
			Throw( type=variables.exceptionType, message="Mutually exclusive arguments: 'csv' and 'filepath'", detail="Only one of either 'filepath' or 'csv' arguments may be provided." );
		if(	csvIsFile ){
			if( !FileExists( arguments.filepath ) )
				Throw( type=variables.exceptionType, message="Non-existant file", detail="Cannot find a file at #arguments.filepath#" );
			if( !isCsvOrTextFile( arguments.filepath ) )
				Throw( type=variables.exceptionType, message="Invalid csv file", detail="#arguments.filepath# does not appear to be a text/csv file" );
	...
	public void function addAutofilter( required workbook, required string cellRange ){
		...
		if( arguments.cellRange.IsEmpty() )
			Throw( type=variables.exceptionType, message="Empty cellRange argument", detail="You must provide a cell range reference in the form 'A1:Z1'" );
	...
	public void function addImage(
		...
		if( !arguments.KeyExists( "filepath" ) AND !arguments.KeyExists( "imageData" ) )
			Throw( type=variables.exceptionType, message="Invalid argument combination", detail="You must provide either a file path or an image object" );
		if( arguments.KeyExists( "imageData" ) AND !arguments.KeyExists( "imageType" ) )
			Throw( type=variables.exceptionType, message="Invalid argument combination", detail="If you specify an image object, you must also provide the imageType argument" );
		var numberOfAnchorElements = ListLen( arguments.anchor );
		if( ( numberOfAnchorElements NEQ 4 ) AND ( numberOfAnchorElements NEQ 8 ) )
			Throw( type=variables.exceptionType, message="Invalid anchor argument", detail="The anchor argument must be a comma-delimited list of integers with either 4 or 8 elements" );
		//we'll need the image type int in all cases
		if( arguments.KeyExists( "filepath" ) ){
			if( !FileExists( arguments.filepath ) )
				Throw( type=variables.exceptionType, message="Non-existent file", detail="The specified file does not exist." );
			try{
				arguments.imageType = ListLast( FileGetMimeType( arguments.filepath ), "/" );
			}
			catch( any exception ){
				Throw( type=variables.exceptionType, message="Could Not Determine Image Type", detail="An image type could not be determined from the filepath provided" );
			}
		}
		else if( !arguments.KeyExists( "imageType" ) )
			Throw( type=variables.exceptionType, message="Could Not Determine Image Type", detail="An image type could not be determined from the filepath or imagetype provided" );
		...
		switch( arguments.imageType ){
			...
			break;
			default:
				Throw( type=variables.exceptionType, message="Invalid Image Type", detail="Valid image types are DIB, EMF, JPG, JPEG, PICT, PNG, and WMF" );
		}
		if( arguments.KeyExists( "filepath" ) ){
			try{
	...
	public void function addPageBreaks( required workbook, string rowBreaks="", string columnBreaks="" ){
		...
		if( arguments.rowBreaks.IsEmpty() AND arguments.columnBreaks.IsEmpty() )
			Throw( type=variables.exceptionType, message="Missing argument", detail="You must specify the rows and/or columns at which page breaks should be added." );
	...
	public void function addRow(
		...
		if( arguments.KeyExists( "row" ) AND ( arguments.row LTE 0 ) )
			Throw( type=variables.exceptionType, message="Invalid row value", detail="The value for row must be greater than or equal to 1." );
		if( arguments.KeyExists( "column" ) AND ( arguments.column LTE 0 ) )
			Throw( type=variables.exceptionType, message="Invalid column value", detail="The value for column must be greater than or equal to 1." );
		if( !arguments.insert AND !arguments.KeyExists( "row") )
			Throw( type=variables.exceptionType, message="Missing row value", detail="To replace a row using 'insert', please specify the row to replace." );
	...
	public void function addRows(
		...
		if( !dataIsQuery && !dataIsArray )
			Throw( type=variables.exceptionType, message="Invalid data argument", detail="The data passed in must be either a query or an array of row arrays." );
		...
		if( dataIsArray && !IsArray( arguments.data[ 1 ] ) )
			Throw( type=variables.exceptionType, message="Invalid data argument", detail="Data passed as an array must be an array of arrays, one per row" );
		var lastRow = getNextEmptyRow( arguments.workbook );
	...
	public void function createSheet( required workbook, string sheetName, overwrite=false ){
		...
		if( !arguments.overwrite )
			Throw( type=variables.exceptionType, message="Sheet name already exists", detail="A sheet with the name '#arguments.sheetName#' already exists in this workbook" );
	...
	public void function formatColumn(
		...
		if( arguments.column LT 1 )
			Throw( type=variables.exceptionType, message="Invalid column value", detail="The column value must be greater than 0" );
		var style = arguments.cellStyle?: buildCellStyle( arguments.workbook, arguments.format );
		var rowIterator = getActiveSheet( arguments.workbook ).rowIterator();
	...
	public any function getCellComment( required workbook, numeric row, numeric column ){
		if( arguments.KeyExists( "row" ) AND !arguments.KeyExists( "column" ) )
			Throw( type=variables.exceptionType, message="Invalid argument combination", detail="If you specify the row you must also specify the column" );
		if( arguments.KeyExists( "column" ) AND !arguments.KeyExists( "row" ) )
			Throw( type=variables.exceptionType, message="Invalid argument combination", detail="If you specify the column you must also specify the row" );
	...
	public struct function getCellFormat( required workbook, required numeric row, required numeric column ){
		if( !cellExists( arguments.workbook, arguments.row, arguments.column ) )
			Throw( type=variables.exceptionType, message="Invalid cell", detail="There doesn't appear to be a cell at row #row#, column #column#" );
	...
	public boolean function isSpreadsheetFile( required string path ){
		if( !FileExists( arguments.path ) )
			Throw( type=variables.exceptionType, message="Non-existent file", detail="Cannot find the file #arguments.path#." );
		try{
	...
	public void function mergeCells(
		...
		if( arguments.startRow LT 1 OR arguments.startRow GT arguments.endRow )
			Throw( type=variables.exceptionType, message="Invalid startRow or endRow", detail="Row values must be greater than 0 and the startRow cannot be greater than the endRow." );
		if( arguments.startColumn LT 1 OR arguments.startColumn GT arguments.endColumn )
			Throw( type=variables.exceptionType, message="Invalid startColumn or endColumn", detail="Column values must be greater than 0 and the startColumn cannot be greater than the endColumn." );
	...
	public any function read(
		...
		if( arguments.KeyExists( "query" ) )
			Throw( type=variables.exceptionType, message="Invalid argument 'query'.", detail="Just use format='query' to return a query object" );
		if( arguments.KeyExists( "format" ) AND !ListFindNoCase( "query,html,csv", arguments.format ) )
			Throw( type=variables.exceptionType, message="Invalid format", detail="Supported formats are: 'query', 'html' and 'csv'" );
		if( arguments.KeyExists( "sheetName" ) AND arguments.KeyExists( "sheetNumber" ) )
			Throw( type=variables.exceptionType, message="Cannot provide both sheetNumber and sheetName arguments", detail="Only one of either 'sheetNumber' or 'sheetName' arguments may be provided." );
		if( !FileExists( arguments.src ) )
			Throw( type=variables.exceptionType, message="Non-existent file", detail="Cannot find the file #arguments.src#." );
	...
	public void function renameSheet( required workbook, required string sheetName, required numeric sheetNumber ){
		...
		if( ( foundAt GT 0 ) AND ( foundAt NEQ sheetIndex ) )
			Throw( type=variables.exceptionType, message="Invalid Sheet Name [#arguments.sheetName#]", detail="The workbook already contains a sheet named [#sheetName#]. Sheet names must be unique" );
	...
	public void function setReadOnly( required workbook, required string password ){
		if( isXmlFormat( arguments.workbook ) )
			Throw( type=variables.exceptionType, message="setReadOnly not supported for XML workbooks", detail="The setReadOnly() method only works on binary 'xls' workbooks." );
	...
	public void function setRepeatingColumns( required workbook, required string columnRange ){
		...
		if( !IsValid( "regex", arguments.columnRange,"[A-Za-z]:[A-Za-z]" ) )
			Throw( type=variables.exceptionType, message="Invalid columnRange argument", detail="The 'columnRange' argument should be in the form 'A:B'" );
	...
	public void function setRepeatingRows( required workbook, required string rowRange ){
		...
		if( !IsValid( "regex", arguments.rowRange,"\d+:\d+" ) )
			Throw( type=variables.exceptionType, message="Invalid rowRange argument", detail="The 'rowRange' argument should be in the form 'n:n', e.g. '1:5'" );
	...
	public void function setSheetPrintOrientation( required workbook, required string mode, string sheetName, numeric sheetNumber ){
		if( !ListFindNoCase( "landscape,portrait", arguments.mode ) )
			Throw( type=variables.exceptionType, message="Invalid mode argument", detail="#mode# is not a valid 'mode' argument. Use 'portrait' or 'landscape'" );
	...
	public void function shiftColumns( required workbook, required numeric start, numeric end=arguments.start, numeric offset=1 ){
		if( arguments.start LTE 0 )
			Throw( type=variables.exceptionType, message="Invalid start value", detail="The start value must be greater than or equal to 1" );
		if( arguments.KeyExists( "end" ) AND ( ( arguments.end LTE 0 ) OR ( arguments.end LT arguments.start ) ) )
			Throw( type=variables.exceptionType, message="Invalid end value", detail="The end value must be greater than or equal to the start value" );
		var rowIterator = getActiveSheet( arguments.workbook ).rowIterator();
	...
	public void function write(
		...
		if( !arguments.overwrite AND FileExists( arguments.filepath ) )
			Throw( type=variables.exceptionType, message="File already exists", detail="The file path specified already exists. Use 'overwrite=true' if you wish to overwrite it." );
		...
		if( passwordProtect AND !engineSupportsWriteEncryption )
			Throw( type=variables.exceptionType, message="Password protection is not supported for Adobe ColdFusion", detail="Password protection currently only works in Lucee, not ColdFusion" );
		if( passwordProtect AND isBinaryFormat( arguments.workbook ) )
			Throw( type=variables.exceptionType, message="Whole file password protection is not supported for binary workbooks", detail="Password protection only works with XML ('xlsx') workbooks." );
		try{
		...
		if( arguments.overwrite OR IsNull( sheet.getRow( JavaCast( "int", arguments.rowNum ) ) ) ){
			try{
				...
				if( exception.message.FindNoCase( "Invalid row number (65536)" ) )
					Throw( type=variables.exceptionType, message="Too many rows", detail="Binary spreadsheets are limited to 65535 rows. Consider using an XML format spreadsheet instead." );
				else
					rethrow;
			}
		}
	...
	private any function createWorkBook(
		...
		if( arguments.streamingXml ){
			...
			Throw( type=variables.exceptionType, message="Invalid 'streamingWindowSize' argument", detail="'streamingWindowSize' must be an integer value greater than 1" );
		...
		*/
		...
		for( var thisRange in ranges ){
			...
			if( !REFind( rangeTest, thisRange ) )
				Throw( type=variables.exceptionType, message="Invalid range value", detail="The range value '#thisRange#' is not valid." );
	...
	private string function generateUniqueSheetName( required workbook ){
		...
		Throw( type=variables.exceptionType, message="Unable to generate name", detail="Unable to generate a unique sheet name" );
	}
	...
	private any function getCellAt( required workbook, required numeric rowNumber, required numeric columnNumber ){
		if( !cellExists( argumentCollection=arguments ) )
			Throw( type=variables.exceptionType, message="Invalid cell", detail="The requested cell [#arguments.rowNumber#,#arguments.columnNumber#] does not exist in the active sheet" );
		var rowIndex = ( arguments.rowNumber -1 );
		var columnIndex = ( arguments.columnNumber -1 );
	...
	private any function getCellValueAsType( required workbook, required cell ){
		...
		if( cellIsOfType( arguments.cell, "FORMULA" ) ){
			...
			try{
				...
				Throw( type=variables.exceptionType, message="Failed to run formula", detail="There is a problem with the formula in sheet #arguments.cell.getSheet().getSheetName()# row #( arguments.cell.getRowIndex() +1 )# column #( arguments.cell.getColumnIndex() +1 )#");
			}
		}
	...
	private struct function getJavaColorRGB( required string colorName ){
		...
		if( IsNull( color[ findColor ] ) OR !IsInstanceOf( color[ findColor ], "java.awt.Color" ) )//don't use member functions on color
			Throw( type=variables.exceptionType, message="Invalid color", detail="The color provided (#arguments.colorName#) is not valid." );
	...
	private numeric function getLastRowNum( required workbook, sheet=getActiveSheet( workbook ) ){ // TODO workbook isn't scoped
	...
	private array function getRowData( required workbook, required row, array columnRanges=[], boolean includeRichTextFormatting=false ){
		...
		if( !arguments.columnRanges.Len() ){
			...
			arguments.columnRanges = [ _columnRange ];
		}
	...
	private any function getSheetByNameOrNumber( required workbook, string sheetName, numeric sheetNumber ){
		...
		if( sheetNameSupplied AND arguments.KeyExists( "sheetNumber" ) )
			Throw( type=variables.exceptionType, message="Invalid arguments", detail="Specify either a sheetName or sheetNumber, not both" );
	...
	private function loadClassUsingJavaLoader( required string javaclass ){
		if( !server.KeyExists( variables.javaLoaderName ) )
			server[ variables.javaLoaderName ] = CreateObject( "component", variables.javaLoaderDotPath ).init( loadPaths=getJarPaths(), loadColdFusionClassPath=false, trustedSource=true );
		return server[ variables.javaLoaderName ].create( arguments.javaclass );
	}
	...
	private void function setCellValueAsType( required workbook, required cell, required value, string type ){
		if( !arguments.KeyExists( "type" ) ) //autodetect type
			...
			Throw( type=variables.exceptionType, message="Invalid data type: '#arguments.type#'", detail="The data type must be one of 'string', 'numeric', 'date' 'boolean' or 'blank'." );
	...
	private void function validateSheetExistsWithName( required workbook, required string sheetName ){
		if( !sheetExists( workbook=arguments.workbook, sheetName=arguments.sheetName ) )
			Throw( type=variables.exceptionType, message="Invalid sheet name [#arguments.sheetName#]", detail="The specified sheet was not found in the current workbook." );
	}
	...
	private void function validateSheetNumber( required workbook, required numeric sheetNumber ){
		if( !sheetExists( workbook=arguments.workbook, sheetNumber=arguments.sheetNumber ) ){
			...
			Throw( type=variables.exceptionType, message="Invalid sheet number [#arguments.sheetNumber#]", detail="The sheetNumber must a whole number between 1 and the total number of sheets in the workbook [#sheetCount#]" );
		}
	}
	...
	private void function validateSheetName( required string sheetName ){
		...
		if( characterCount GT 31 )
			Throw( type=variables.exceptionType, message="Invalid sheet name", detail="The sheetname contains too many characters [#characterCount#]. The maximum is 31." );
		var poiTool = loadClass( "org.apache.poi.ss.util.WorkbookUtil" );
		try{
			poiTool.validateSheetName( JavaCast( "String", arguments.sheetName ) );
		}
		catch( "java.lang.IllegalArgumentException" exception ){
			Throw( type=variables.exceptionType, message="Invalid characters in sheet name", detail=exception.message );
		}
	...
	private void function validateSheetNameOrNumberWasProvided(){
		if( !arguments.KeyExists( "sheetName" ) AND !arguments.KeyExists( "sheetNumber" ) )
			Throw( type=variables.exceptionType, message="Missing Required Argument", detail="Either sheetName or sheetNumber must be provided" );
		if( arguments.KeyExists( "sheetName" ) AND arguments.KeyExists( "sheetNumber" ) )
			Throw( type=variables.exceptionType, message="Too Many Arguments", detail="Only one argument is allowed. Specify either a sheetName or sheetNumber, not both" );
	}
	...
	private string function fontStyleToCss( required string styleType, required any styleValue, workbook ){
		...
		*/
		switch( arguments.styleType ){
			...
			case "color":
				if( !arguments.KeyExists( "workbook" ) )
					Throw( type=variables.exceptionType, message="The 'workbook' argument is required when generating color css styles" );
		...
		Throw( type=variables.exceptionType, message="Unrecognised style for css conversion" );
	}
