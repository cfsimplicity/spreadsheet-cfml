component accessors="true"{
	...
	/* TODO if more than one footer/header image is added, xlsx won't open in MS Excel */
	private void function setHeaderOrFooterImage(
		...
		var partName = loadClass( "org.apache.poi.openxml4j.opc.PackagingURIHelper" ).createPartName( "/xl/drawings/#headerType.LCase()#Image#vmlPosition#.vml" );
		...
		headerImageVml
			.setRIdPic( pictureRelationID )
	  	.setImageDimension( imageDimension )
	  	.setPosition( vmlPosition )
	  ;
	  //create the sheet/vml relation
	  var xssfVmlRelation = loadClass( "org.apache.poi.xssf.usermodel.XSSFRelation" ).VML_DRAWINGS;
  	rIdExtLink = sheet.addRelation( JavaCast( "null", 0 ), xssfVmlRelation, headerImageVml ).getRelationship().getId();
