component{
	...
	public void function setCellValue( required workbook,required value,required numeric row,required numeric column, string default_type="" ){
		//Automatically create the cell if it does not exist, instead of throwing an error
		var cell = initializeCell( workbook, row, column );
		setCellValueAsType( workbook, cell, value, default_type );
	}
	...
	private void function setCellValueAsType( required workbook, required cell, required value,  string default_type="" ){
		if( IsNumeric( value ) AND !REFind( "^0[\d]+", value ) AND ( default_type=="numeric" || default_type=="" ) ){ /*  skip numeric strings with leading zeroes. treat those as text  */
			/*  NUMERIC  */
			cell.setCellType( cell.CELL_TYPE_NUMERIC );
			cell.setCellValue( JavaCast( "double", Val( value ) ) );
			return;
		}
		if( IsDate( value ) AND ( default_type=="date" || default_type=="" ) ){
			/*  DATE  */
		...
		if( IsBoolean( value ) AND ( default_type=="boolean" || default_type=="" ) ){
			/* BOOLEAN */
